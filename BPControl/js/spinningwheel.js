...
var SpinningWheel = {
	...
	create: function () {
		...
		div.innerHTML = '<div id="sw-header"><div id="sw-cancel">Cancelar</' + 'div><div id="sw-done">Confirmar</' + 'div></' + 'div><div id="my-header"><div id="sw-pas">PAS</' + 'div><div id="sw-pad">PAD</' + 'div><div id="sw-pulse">PULSE</' + 'div></' + 'div><div id="sw-slots-wrapper"><div id="sw-slots"></' + 'div></' + 'div><div id="sw-frame"></' + 'div>';
	...
	open: function () {
		...
		this.swWrapper.style.webkitTransform = 'translate3d(0, -285px, 0)';
	},
	...
	getSelectedValues: function () {
		...
		for (i in this.slotEl) {
			...
			for (l in this.slotData[i].values) {
				if (count == index) {
					if(t==0){
						sist.push(this.slotData[i].values[l]);
						t=1;
					}else if (t==1){
						dist.push(this.slotData[i].values[l]);
						t=2;
					}else{
						pols.push(this.slotData[i].values[l]);
					}
					break;
				}
				count += 1;
		...
		return { 'sist':sist,'dist':dist, 'pols':pols};
	},
