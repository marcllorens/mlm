...
var SpinningWheel = {
	...
	getSelectedValues: function () {
		...
		for (i in this.slotEl) {
			...
			for (l in this.slotData[i].values) {
				if (count == index) {
					...
					sist.push(this.slotData[1].values[l]);
					dist.push(this.slotData[2].values[l]);
					pols.push(this.slotData[3].values[l]);
					break;
				}
		...
		return { 'keys': keys, 'values': values, 'sist':sist,'dist':dist, 'pols':pols };
	},
